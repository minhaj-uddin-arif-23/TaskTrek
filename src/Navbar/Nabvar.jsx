    ...
    <nav className=" shadow-md fixed top-0 left-0 w-full  z-50 px-36 ">
      <div className="container mx-auto flex justify-between items-center p-4">
        ...
        <div className="hidden md:flex space-x-6">
          <p className="mt-1 cursor-pointer" onClick={change}>
            {state == "light-mode" ? <FaMoon /> : <CiLight />}
          </p>
          <Link to={"/"} className=" hover:text-blue-600 transition">
            Home
          </Link>
          <Link to={"/alltask"} className=" hover:text-blue-600 transition">
            All Tasks
          </Link>
          ...
          {user && user?.email ? (
            <>
              <button
                onClick={signout}
                className=" hover:text-blue-600 transition  px-5  mb-4 cursor-pointer"
              >
                Sign out
              </button>
              <div>
      ...
      {isOpen && (
        <div className="md:hidden bg-white shadow-md flex flex-col items-center py-4">
          <Link
            to={"/"}
            className="text-gray-700 py-2 hover:text-blue-600 transition"
          ...
          {user && user?.email ? (
            <button
              onClick={signout}
              className="text-gray-700 hover:text-blue-600 transition border-2 border-gray-200 px-3 rounded-lg"
            >
              Sign out
            </button>
          ) : (
